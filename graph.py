...
class ImportCSVInput(BaseModel):
    csv_file_path: str = Field(description="CSV文件的路径，例如 'telco_data.csv'")
    table_name: str = Field(description="要导入数据的MySQL表名，例如 'telco_churn'")

@tool(args_schema=ImportCSVInput)
def import_csv_to_mysql(csv_file_path: str, table_name: str) -> str:
    """
    将指定的CSV文件数据导入到MySQL数据库的指定表中。
    会先读取 `create_table.sql` 创建或确保表结构存在。
    """
    load_dotenv(override=True)
    host = os.getenv('HOST')
    user = os.getenv('USER')
    ...
    try:
        ...
        with connection.cursor() as cursor:
            # 读取并执行 create_table.sql
            with open('create_table.sql', 'r', encoding='utf8') as f:
                create_table_sql = f.read()
            # 分割语句并执行，因为可能包含多个语句（如DROP TABLE和CREATE TABLE）
        ...
        # 处理 TotalCharges 列：空字符串替换为 NaN，然后转换为数值类型
        df['TotalCharges'] = pd.to_numeric(df['TotalCharges'], errors='coerce')
        # 将 NaN 替换为 0 或其他默认值，具体根据业务需求决定
        df['TotalCharges'] = df['TotalCharges'].fillna(0) 

        # 导入数据到 MySQL
        # 使用 pymysql engine，并设置 chunksize 防止内存溢出
        df.to_sql(name=table_name, con=connection, if_exists='append', index=False, chunksize=1000)
        connection.commit()
        return f"✅ '{csv_file_path}' 数据已成功导入到 MySQL 表 '{table_name}'。"
...
# ✅ 创建工具列表
tools = [search_tool, python_inter, fig_inter, sql_inter, extract_data, import_csv_to_mysql]
